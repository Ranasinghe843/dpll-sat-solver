...
def parse_cnf_clauses(filepath):
    """Parses DIMACS CNF into a list of sets for mathematical verification."""
    clauses = []
...
def verify_assignment(clauses, assignment_dict):
    """Manually checks if the assignment satisfies every clause in the CNF."""
...
def run_user_solver(cnf_path):
    """Runs mySAT.py and parses the custom no-space output format."""
    try:
...
def run_minisat(cnf_path):
    """Runs MiniSat as the ground truth for UNSAT verification."""
    # MiniSat Return Codes: 10 = SAT, 20 = UNSAT
...
def run_full_suite(root_folder):
    ...
    print(f"{'='*75}")
    print(f"ULTIMATE VERIFICATION SUITE: {root_folder}")
    print(f"{'='*75}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 verify_results.py <dataset_folder>")
    else:
